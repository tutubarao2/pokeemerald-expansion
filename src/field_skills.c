...
void FieldSkillReveal(void)
{
    ...
    if (ItemfinderCheckForHiddenItems(eventos, TASK_NONE))
    {
        for (i = 0; i < eventos->bgEventCount; i++)
        {
                ...
                && !FlagGet(eventos->bgEvents[i].bgUnion.hiddenItem.hiddenItemId + FLAG_HIDDEN_ITEMS_START))
            {
                ...
                if (distanceX >= -7 && distanceX <= 7 && distanceY >= -5 && distanceY <= 5)
                {
                    ...
                    {
                        objectEventId = SpawnSpecialObjectEventParameterized(OBJ_EVENT_GFX_POKE_BALL, MOVEMENT_TYPE_LOOK_AROUND,
                        LOCALID_NONE, (s16)itemX, (s16)itemY, ELEVATION_DEFAULT);
                    }
                    else
                    {
                        // remove o item spawnado
                        RemoveObjectEvent(&gObjectEvents[objectEventId + 1]);
                    }
